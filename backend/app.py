...
async def test_smtp_live(email: str = None):
    import smtplib
    ...
    if brevo_key:
        ...
        payload = {
            ...
            "textContent": f"This is a Brevo API connection test from Render to {target_email}."
        }
        try:
            async with httpx.AsyncClient() as client:
                ...
                if res.status_code in [200, 201, 202]:
                    ...
                    return {"status": "success", "message": f"Email sent successfully via Brevo HTTPS API to {target_email}!", "steps": steps}
                else:
    ...
    if resend_key:
        ...
        payload = {
            ...
            "text": f"This is a Resend API connection test from Render to {target_email}."
        }
        try:
            async with httpx.AsyncClient() as client:
                res = await client.post(url, json=payload, headers=headers, timeout=10)
                if res.status_code in [200, 201, 202]:
                    ...
                    return {"status": "success", "message": f"Email sent successfully via Resend HTTPS API to {target_email}!", "steps": steps}
                else:
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"SMTP Connection and sending succeeded completely to {target_email}!",
            "steps": steps
        }
    except Exception as e:
